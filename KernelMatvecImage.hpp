...
public:
    ...
    bool checkOutput(OCLApp& oclApp, const bool printOutput) {
        if (_paranoidCheck) {
            return generalizedMatvec()
                       ? checkBuffer<scalar>(oclApp, _handleC, dimM(), _paranoidC, printOutput)
                       : checkImage<scalar>(oclApp, _handleC, dimM(), _paranoidC, printOutput);
        } else {
            const scalar testValue = dimN();
            return generalizedMatvec()
                       ? checkBuffer<scalar>(oclApp, _handleC, dimM(), testValue, printOutput)
                       : checkImage<scalar>(oclApp, _handleC, dimM(), testValue, printOutput);
        }
    }
    ...
    bool setArgs(OCLApp& oclApp, const size_t kernelHandle, const bool syncInput) {
	    ...
	    oclApp.releaseImages();
            ...
            _handleA = transposeA()
                           ? createImageR<scalar>(oclApp, dimM(), dimN(), "matA", 1)
                           : createImageR<scalar>(oclApp, dimN(), dimM(), "matA", 1);
            _handleB = createImageR<scalar>(oclApp, VECTOR_LENGTH, dimN() / VECTOR_LENGTH, "vecB", 1);
            _handleC = generalizedMatvec()
                           ? createBufferRW<scalar, VECTOR_LENGTH>(oclApp, dimM(), "vecC", 0)
                           : createImageW<scalar>(oclApp, VECTOR_LENGTH, dimM() / VECTOR_LENGTH, "vecC", 0);
        ...
        if (_paranoidCheck) {
            ...
            if (fillrandImage<scalar>(oclApp, _handleA, dimM() * dimN()) &&
                fillrandImage<scalar>(oclApp, _handleB, dimN()) &&
                (generalizedMatvec()
    ...
    std::ostream& print(std::ostream& os) const {
        ...
        if (generalizedMatvec()) {
            const ConstantValue<std::string> outC = vecC_buf + wholeHeight() * globalRow;
