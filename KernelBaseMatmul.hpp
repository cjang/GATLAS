...
protected:
    ...
    template <typename T>
    bool checkArray(const T *ptr,
                    const size_t length,
                    const T testValue) const {
    ...
    bool clearBuffer(OCLApp& oclApp,
                     const size_t bufferIndex,
                     const scalar value = 0);

    bool clearImage(OCLApp& oclApp,
                    const size_t imageIndex);

    bool fillrandBuffer(OCLApp& oclApp,
                        const size_t bufferIndex,
                        const size_t length);

    bool fillrandImage(OCLApp& oclApp,
                       const size_t imageIndex,
                       const size_t length);
